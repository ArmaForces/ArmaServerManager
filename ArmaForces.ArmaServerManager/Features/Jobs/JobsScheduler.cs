...
namespace ArmaForces.ArmaServerManager.Features.Jobs
{
    ...
    internal class JobsScheduler : IJobsScheduler
    {
        ...
        /// <inheritdoc cref="IJobsScheduler" />
        public Result<int, IError> ScheduleJob<T>(Expression<Func<T, Task>> func, DateTime? dateTime = null) where T : class
            => dateTime.HasValue
                ? ScheduleAt(func, dateTime.Value)
                : EnqueueImmediately(func);

        /// <inheritdoc cref="IJobsScheduler"/>
        public Result<int, IError> ContinueJobWith<T>(int parentId, Expression<Func<T, Task>> func) where T : class
        ...
        /// </summary>
        private Result<int, IError> ScheduleAt<T>(Expression<Func<T, Task>> func, DateTime dateTime) where T : class
        {
            ...
            if (similarScheduledJob is not null)
                return Result.Failure<int, IError>(new Error($"Similar job is already scheduled at {similarScheduledJob.EnqueueAt}.", ManagerErrorCode.SimilarJobFound))
            ...
            if (dateTime.IsCloseTo(DateTime.Now, _defaultPrecision))
            {
                ...
                if (queuedJobs.Any())
                    return Result.Failure<int, IError>(new Error($"Similar job is already in queue.", ManagerErrorCode.SimilarJobFound))
        ...
        /// </summary>
        private Result<int, IError> EnqueueImmediately<T>(Expression<Func<T, Task>> func) where T : class
        {
