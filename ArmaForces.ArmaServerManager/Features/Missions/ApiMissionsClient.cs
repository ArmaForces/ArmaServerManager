...
namespace ArmaForces.ArmaServerManager.Features.Missions
{
    ...
    internal class ApiMissionsClient : HttpClientBase, IApiMissionsClient
    {
        ...
        /// <inheritdoc />
        public async Task<Result<List<WebMission>, IError>> GetUpcomingMissions()
            => await ApiMissionsUpcoming();

        /// <inheritdoc />
        public async Task<Result<HashSet<string>, IError>> GetUpcomingMissionsModsetsNames()
        {
        ...
        private Result<HashSet<string>, IError> GetMissionModsets(IReadOnlyCollection<WebMission> missions)
            => missions
                .GroupBy(x => x.Modlist)
                .Select(x => x.First())
                .Select(x => x.Modlist)
        ...
        private async Task<Result<List<WebMission>, IError>> ApiMissionsUpcoming() {
